...
echo ""
echo "  Stage drill-downs (Sections 6–8):"
echo "    • Hooks with high 'time' and many callbacks are worth investigating"
echo "    • Note the source — plugin name usually appears in the callback or hook name"
echo ""
echo "  Spotlight (Sections 9–10):"
echo "    • These are your actionable items — hooks/callbacks taking ≥1ms of real time"
echo "    • Section 10 covers all stages and is the most useful starting point"
echo "    • Cross-reference with Section 4 of wp-perf-diag.php (plugins list)"
echo ""
echo "  Hook spotlights (Sections 11–17):"
echo "    • Per-hook breakdown of slow callbacks for init, plugins_loaded, wp_head, etc."
echo "    • 'wp_head' and 'wp_footer' are expected to be busy; look for surprises"
echo "    • 'pre_get_posts' callbacks that run slow queries are a common culprit"
# ── Save report ───────────────────────────────────────────────
